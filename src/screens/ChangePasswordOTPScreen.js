 ...
 * Enter 6-digit OTP → Verify → navigate to Change Password Reset
 */
...
const ChangePasswordOTPScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const stored = await AsyncStorage.getItem('change_password_email');
        if (stored) setEmail(stored);
        else navigation.replace('ChangePassword');
      } catch {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      navigation.navigate('ChangePasswordReset');
    } catch (err) {
